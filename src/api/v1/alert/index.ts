...
router.post('/', authenticate, validate(validateCreateEthAlertSchema), CreateEthAlertController);

router.get('/', authenticate, validate(validateListAlertSchema, 'query'), ListAlertsControllers);

router.get('/:id', authenticate, validate(validateGetAlertById, 'params'), GetAlertByIdController);
